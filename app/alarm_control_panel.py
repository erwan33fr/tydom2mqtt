...
class Alarm:
    ...
    async def update(self):
        ...
        if (self.mqtt is not None and self.current_state is not None):
            self.mqtt.mqtt_client.publish(
                self.state_topic,
                self.current_state,
                qos=0,
    ...
    async def put_alarm_state(tydom_client, device_id, alarm_id, home_zone, night_zone, asked_state=None):
        ...
        # la plateforme Alarm Control Panel de Home Assistant en configurer pour pouvoir générer des payloads MQTT 
        # ARM_AWAY, ARM_HOME, ARM_NIGHT. En fonction de l'état demandé on définit l'état demandé ON ou OFF et
        # la zone concerné si la payload n'est pas ARM_AWAY.
        # ARM_NIGHT n'est pas correctement géré dans le retour d'état. Il faut utiliser uniquement ARM_AWAY, et ARM_HOME
        # Pour gérer l'acquitement des défauts, dans Home Assistant, un bouton d'acquitement générant une payload ACK a été ajouté
        ...
        await tydom_client.put_alarm_state(device_id=device_id, alarm_id=alarm_id, value=value, zone_id=zone_id)
