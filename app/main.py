...
try:
    with open(data_options_path) as f:
        logger.info(
            f"{data_options_path} detected ! parsing options.json...."
        )
        try:
            data = json.load(f)
            logger.debug(data)
    ...
    logger.info(f"No {data_options_path}")
    # CREDENTIALS TYDOM
...
# déclaration de la connexion au tydom
...
# déclaration de la connexion MQTT à Home Assistant
hassio = MQTT_Hassio(
